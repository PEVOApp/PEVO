//
//  OHSheltersTableViewController.swift
//  Pevo
//
...
class OHSheltersTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating, UISearchBarDelegate, SearchControllerDelegate {
    
    @IBOutlet var shelterSearchView: UITableView!
    
    var ohshelter = Array<OHShelters>()
    
    var filteredShelters = Array<OHShelters>()
    ...
    func loadListShelterSearch() {
        ...
        let path = Bundle.main.path(forResource: "OhioShelterSearch", ofType: "plist")!
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as! OhioTableViewCell
        
        // Configure the cell...
        let ohshelters = (shouldShowSearchResults) ? filteredShelters[indexPath.row] : ohshelter[indexPath.row]
        cell.updateForOhShelter(ohshelters)
    ...
    func filterContentForSearchText(_ searchText: String?) {
        if let someSearchText = searchText {
            filteredShelters = ohshelter.filter({ ( ohshelter: OHShelters) -> Bool in
                return ohshelter.cityName.contains(someSearchText) || ohshelter.shelterName.contains(someSearchText) || ohshelter.stateName.contains(someSearchText)
            })
        }
        else {
            filteredShelters = ohshelter
        }
    }
