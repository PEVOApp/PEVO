...
class MNShelters: NSObject {
    // MARK: Initialization
    convenience init(plistValues: Dictionary<String, String>) throws {
        guard let cityName = plistValues[MNShelters.CityKey],let stateName = plistValues[MNShelters.StateKey] ,let shelterName = plistValues[MNShelters.NameKey], let phoneNumber = plistValues[MNShelters.PhoneNumberKey] else {
            let errorInfo = [NSLocalizedDescriptionKey : "Shelter plist values did not contain expected city, name, and phone number values for the keys \(MNShelters.CityKey), \(MNShelters.StateKey), \(MNShelters.NameKey), and \(MNShelters.PhoneNumberKey)"]
